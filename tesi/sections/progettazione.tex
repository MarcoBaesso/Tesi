...
while (activityPlanVersion.hasNext()){
	...
	//assignmentsVersion ha tipo Iterator e contiene gli assegnamenti predisposti per l'attività activityVersion
	if (assignmentsVersion.hasNext){
		...
		//salvataggio nella variabile time l'ora di inizio dell'attività
		Timestamp time=new Timestamp(activityVersion.getStart().getTime();)
		Integer durata=0;
		//il ciclo continua fino a quando esiste almeno una risorsa il cui impegno residuo è maggiore di 0
...
Il diagramma di Gantt consentiva di impostare la granularità in giorni, settimane o mesi; è stata aggiunta la visualizzazione delle attività in ore. La classe che ha il compito di costruire il diagramma di Gantt figura con il nome di \textit{OpProjectComponent.java}; un oggetto dichiarato con tale tipo presenta tra le proprietà le dimensioni dell'oggetto grafico rappresentante l'attività interessata, le dimensioni includono posizionamento rispetto all'asse verticale e orrizzontale; l'asse verticale coincide con la data di inizio del piano di progetto. Il piano rappresentante il diagramma di Gantt presenta una dimensione fissa e non ridimensionabile e il posizionamento degli oggetti grafici avviene in base alla granulatità impostata; per esempio se per la granularità in giorni vengono visualizzati i giorni appunto, viene presa la dimensione che è fissa della larghezza di un giorno e in base a questa viene costruito l'oggetto di tipo \textit{OpProjectComponent} considerando data di inizio e fine dell'attività. Per la visualizzazione in ore è quindi necessario aggiungere un nuovo asse orrizzontale in cui vengono visualizzate le ore del giorno e integrare il calcolo per stabilire le dimensioni dell'oggetto grafico in relazione alla granularità in ore nel seguente metodo: \\
...
Occorre gestire il ridimensionamento dell'oggetto grafico nel caso in cui sia stata impostata come granularità l'ora. Il metodo che intercetta l'evento di riposizionamento è il seguente:\\
\textit{public void processPointerEvent(HashMap event, int action, int x, int y, int modifiers)};\\
questo metodo presenta degli switch case in base al tipo di oggetto \textit{OpProjectComponent} e in base al tipo di attività a cui il componente grafico si riferisce (milestone, \textit{\textbf{standard\_activity}}, \textit{\textbf{collection\_activity}}). Il case che necessita di modifiche è POINTER\_DRAG\_END; qui è gestito sia l'evento relativo allo spostamento della data di inizio e della data di fine e quindi qui vanno apportati gli aggiustamenti per la modifica dell'ora di inizio e di fine.  